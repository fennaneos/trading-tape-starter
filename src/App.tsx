...
function TopNav() {
  ...
  return (
    ...
    >
      <img
        src="https://dummyimage.com/28x28/222/fff&text=¥"
        width={28}
        height={28}
        style={{ borderRadius: 8 }}
      />
      <div className="brand" style={{ fontWeight: 800, color: '#ffd700' }}>
        Trading Core
      </div>
      <div style={{ flex: 1 }} />
      <div className="nav-links hide-mobile" style={{ display: 'flex', gap: 10 }}>
        ...
        <Link to="/ai" className="gb-btn gb-outline">
          AI
        </Link>
        <Link to="/portfolio" className="gb-btn gb-outline">
          Portfolio
        </Link>
...
function Dashboard({ apiBase }: { apiBase: string }) {
  ...
  const [annotations, setAnnotations] =
    useState<{ level: number; type: 'support' | 'resistance' }[]>([]);
  ...
  const onDel = async (s: string) => {
    try {
      const res = await fetch(`${apiBase}/api/user/watchlist/${encodeURIComponent(s)}`, {
        method: 'DELETE',
      });
      if (res.ok) {
        const j = await res.json();
  ...
  const toggleRSI = (v: boolean) => requirePro(() => {
    setRsiPane(v);
    if (v) setMacdPane(false);
  });
  const toggleMACD = (v: boolean) => requirePro(() => {
    setMacdPane(v);
    if (v) setRsiPane(false);
  });
  const toggleBB = (v: boolean) => requirePro(() => setShowBB(v));

  /* ---- Clear annotations on symbol/tf change ---- */
  useEffect(() => setAnnotations([]), [symbol, tf]);

  /* ---- Mobile quick bar (selects) ---- */
  ...
  return (
    <>
      <div className="page charts" style={{ paddingBottom: 72 }}>
        <div
        ...
        >
              ...
              {list.map((it) => (
                <div className="item" key={it.symbol}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <button className="gb-btn gb-outline" onClick={() => setSymbol(it.symbol)}>
                      {it.symbol}
                    </button>
                    <span className="badge">{it.name || it.symbol}</span>
                  </div>
                  <button className="gb-btn gb-ghost" onClick={() => onDel(it.symbol)}>
                    Remove
                  </button>
                </div>
              ))}
          ...
          <div className="panel">
            ...
            <div className="panelBody">
              <div className="controls">
                <div className="controls-row">
                  <label className="label">
                    Symbol <HelpTip title="Symbol" body="Choose the market to visualize." />
                  </label>
                  <select value={symbol} onChange={(e) => setSymbol(e.target.value as any)}>
                    {list.map((it) => (
                      <option key={it.symbol} value={it.symbol}>
                  ...
                  <label className="label">
                    TF <HelpTip title="Timeframe" body="Candle compression from 1 minute to 1 day." />
                  </label>
                  <select value={tf} onChange={(e) => setTf(e.target.value as any)}>
                    {['1m', '5m', '15m', '1h', '4h', '1d'].map((t) => (
                      <option key={t} value={t}>
                  ...
                  <label className="label">
                    Mode <HelpTip title="Chart mode" body="Line, Candles, or a hybrid Both." />
                  </label>
                  <select value={mode} onChange={(e) => setMode(e.target.value as any)}>
                    {['both', 'candles', 'line'].map((m) => (
                      <option key={m} value={m}>
                ...
                <div className="controls-row">
                  ...
                  <span className="label">
                    EMA Fast <HelpTip title="EMA Fast" body="Shorter lookback exponential moving average." />
                  </span>
                  <input
                    className="num"
                    type="number"
                    value={emaFast}
                    onChange={(e) => setEmaFast(parseInt(e.target.value || '0') || 0)}
                  />
                  <span className="label">
                    EMA Slow <HelpTip title="EMA Slow" body="Longer lookback exponential moving average." />
                  </span>
                  <input
                    className="num"
                    type="number"
                    value={emaSlow}
                    onChange={(e) => setEmaSlow(parseInt(e.target.value || '0') || 0)}
                  />

                  <label className="toggle">
                    <input type="checkbox" checked={showBB} onChange={(e) => toggleBB(e.target.checked)} />
                    Bollinger{' '}
                    <HelpTip title="Bollinger Bands" body="Volatility bands around a moving average. Unlocks with Pro." />
                  </label>
                  <span className="label">
                    BB Period <HelpTip title="BB Period" body="Lookback window for the bands." />
                  </span>
                  <input
                    className="num"
                    type="number"
                    value={bbPeriod}
                    onChange={(e) => setBbPeriod(parseInt(e.target.value || '20') || 20)}
                  />
                  <span className="label">
                    k <HelpTip title="Band width" body="Standard deviation multiplier (typically 2)." />
                  </span>
                  <input
                    className="num"
                    type="number"
                    value={bbK}
                    onChange={(e) => setBbK(parseFloat(e.target.value || '2') || 2)}
                  />
                </div>
              </div>
              ...
              <MiniChartPro
                ...
                crosshair={true} // ensure cursor/crosshair is on
              />
          ...
          {/* Right column: AI + Backtest + Upgrade */}
          <div className="right-col" style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            {/* Backtest (always available as a teaser; we can later gate advanced params under Pro) */}
            <BacktestPanel
              apiBase={API_BASE}
              symbol={symbol}
              tf={tf}
...
function AiPage({ apiBase }: { apiBase: string }) {
  ...
  return (
    <div className="page" style={{ padding: 12, paddingBottom: 72 }}>
      <div className="panel">
        <div className="panelHeader">AI Market Assistant</div>
        <div className="panelBody">
          <div className="controls">
